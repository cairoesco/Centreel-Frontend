...
export class ViewProductComponent implements OnInit {
  ...
  auto_generate_barcode_variant(index) {
    const control = (<FormArray>this.variantsForm.controls["variant_detail"])
      .at(index)
      .get("barcode") as FormArray;
  ...
  subCategory(evt, event) {
    if (event.isUserInput) {
      this.categoryList = this.rawDetail.product_types.find(
        (x) => x.type_id === evt
      );
  ...
  generalInfoForm(product) {
    if (product.product_thc) var thc = product.product_thc.split("-");
  ...
  public fileDragged(type) {
    this.uploader.queue.forEach((element) => {
      ...
      if (
        ...
        this.utility.showSnackBar("Unsupported file format", {
          panelClass: "error",
        });
      }
    });
  ...
  countProductProperties(event, data, index) {
    const propertyControl: any =
      this.propertiesForm.controls["productProperties"];
    const control: any = propertyControl
      .at(index)
      .get("selected_product_attribute_properties");
    if (event.isUserInput) {
      if (event.source._selected) {
        control.push(this.addSelectedProductAttributeProperties(data));
      } else {
        const tempData = control.value;
        let updateItem = _.find(tempData, {
          attribute_property_id: data.property_id,
        });
  ...
  viewDetailVariant(variant_id) {
    this.router.navigateByUrl(
      "products/" + this.product_id + "/variants/" + variant_id + "/view"
    );
  }
  ...
  countProvienceInfo(event, data) {
    ...
    if (event.isUserInput) {
      if (event.source._selected) {
        ...
        const tempData =
          this.identificationForm.get("product_proviences").value;
  ...
  private variantData(data) {
    return this.formBuilder.group({
      ...
      batch_no: [
        data.variant_batches.length > 0 ? data.variant_batches[0].batch_no : "",
      ],
  ...
  viewSupplier(data, index) {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        let supplierIndex = this.supplierListData.indexOf(
          this.supplierForm.controls.product_suppliers_data.value[index]
        );
        this.supplierForm.controls.product_suppliers_data.value[index] =
          result.data[0];
        this.supplierListData[supplierIndex] = result.data[0];
        this.supplierForm.controls.product_suppliers_data.setValue(
          this.supplierForm.controls.product_suppliers_data.value
        );
      }
    });
  }
  ...
  getProvince() {
    this.api.getProvinceList().subscribe((response: any) => {
      if (response.success) {
  ...
  getCityList(parent) {
    this.api.getCityList(parent).subscribe((response: any) => {
      if (response.success) {
  ...
  getRawDetails_temp(data) {
    ...
    this.rawDetail.product.product_attributes.forEach((element) => {
      this.rawDetail.product_attributes.forEach(
        (parentelement, parentIndex) => {
          let attr__property_control: any =
            this.propertiesForm.controls["productProperties"];
          let attr_control = attr__property_control
            .at(parentIndex)
            .get("property_id");
          let control: any;
          control = attr__property_control
            .at(parentIndex)
            .get("selected_product_attribute_properties");
          parentelement.product_attribute_properties.forEach(
            (childelement, index) => {
              if (element.attr_property_id == childelement.property_id) {
                ...
                control.push(
                  this.editSelectedProductAttributeProperties(element)
                );
              }
            }
          );
  ...
  public result: any;
  getProductById() {
    this.api.getProductById(this.product_id).subscribe((response: any) => {
      if (response.success) {
        ...
        this.cannabiesData = response.data.product_types.find(
          (x) => x.type_id === this.productData.type_id
        );
        this.isCanabies = this.cannabiesData.type_slug == "cannabis";
        this.dynamicHeight =
          this.variants.length < 12
        ...
        this.getRawDetails_temp({
          ...
          variant_properties: response.data.variant_properties,
        ...
        const control1: any =
          this.identificationForm.controls["product_proviences"];
        this.variantsForm = this.fb.group({
          variant_detail: this.formBuilder.array([]),
        });
        const variant_control: any =
          this.variantsForm.controls["variant_detail"];
  ...
  generalInfoStore(event) {
    ...
    var product_thc =
      this.generalForm.get("thc1").value +
      "-" +
      this.generalForm.get("thc2").value;
    var product_cbd =
      this.generalForm.get("cbd1").value +
      "-" +
      this.generalForm.get("cbd2").value;
  ...
  deleteImagesArray(file, index) {
    if (index !== -1 && file == 0) {
      let file_index =
        index - (this.arrayOfImages.length - this.imagesOfarray.length);
  ...
  identificationInfoStore(event) {
    ...
    Object.keys(this.identificationForm.value).forEach((key) => {
      if (this.identificationForm.value[key] instanceof Object) {
        formData.append(
          key,
          JSON.stringify(this.identificationForm.value[key])
        );
      } else {
  ...
  supplierInfoStore(event) {
    ...
    let suppliers_ids =
      this.supplierForm.controls.product_suppliers_data.value.map((x) => x.id);
  ...
  propertyInfoStore(event) {
    ...
    this.propertiesForm.get("productProperties").value.forEach((element) => {
      if (
        element.selected_product_attribute_properties &&
        element.selected_product_attribute_properties.length > 0
      ) {
  ...
  updateProduct(data) {
    this.api.updateProduct(data, this.product_id).subscribe(
      (response: any) => {
        if (response.success) {
  ...
  onVariantSubmit(id, index) {
    ...
    if (data.stocks_variants_id < 1) {
      this.utility.showSnackBar(
        "No Stock is added for " +
          data.variant_name +
          " variant, kindly add stock.",
        { panelClass: "error" }
      );
      selling_error = true;
    }
    ...
    if (data.purchase_price > data.variant_price) {
      this.utility.showSnackBar(
        "selling price must be equal or greater than purchase price for " +
          data.variant_name,
        { panelClass: "error" }
      );
      selling_error = true;
    ...
    formData.append(
      ...
      JSON.stringify({
        ...
        chain_id: this.rawDetail.product.chain_id,
      })
    );
    ...
    if (control1.valid && !selling_error && data.store_id > 0) {
      this.api.updateVariant(formData, id).subscribe(
        (response: any) => {
          if (response.success) {
            this.utility.showSnackBar(response.message);
            this.isSubmitted = false;
