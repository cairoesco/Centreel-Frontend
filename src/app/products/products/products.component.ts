...
export class ProductsComponent implements OnInit {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (Boolean(result)) {
        ...
        result.product_type_id
          ? (this.productobj["product_type_id"] = result.product_type_id
              ? JSON.stringify(result.product_type_id)
              : "")
          : delete this.productobj["product_type_id"];
        result.product_category_id
          ? (this.productobj["product_category_id"] = result.product_category_id
              ? JSON.stringify(result.product_category_id)
              : "")
          : delete this.productobj["product_category_id"];

        result.store_id
          ? (this.productobj["store_id"] = result.store_id
              ? JSON.stringify(result.store_id)
              : "")
          : delete this.productobj["store_id"];

        result.tags
          ? (this.productobj["tags"] = result.tags
              ? JSON.stringify(result.tags)
              : "")
          : delete this.productobj["tags"];
  ...
  getVariantData() {
    this.productService
      .GetProductList(this.productobj)
      .subscribe((response: any) => {
        ...
        if (response.success) {
          ...
          this.dynamicHeight =
            this.newrows.length < 12
              ? (this.newrows.length + 1) * 48 + 140 + "px"
              : "";
        } else {
  ...
  AddNewTag(productdetail): void {
    if (
      (productdetail == "ALL" && this.selected.length > 0) ||
      Boolean(productdetail.variant_id)
    ) {
      let ids = this.selected.map(
        ({ variant_id: id, variant_id, ...rest }) => ({
          id,
          variant_id,
          ...rest,
        })
                ...
                if (
                  !Boolean(
                    _.find(
                      ...
                      function (o) {
                        return (
                          o.tag_name.toLowerCase() == tag_name.toLowerCase()
                        );
                      }
                    )
                  )
