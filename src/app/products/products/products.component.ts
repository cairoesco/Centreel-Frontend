...
export class ProductsComponent implements OnInit {
	...
	constructor(public dialog: MatDialog, private productService: ProductService, private utils: UtilsServiceService, private el: ElementRef) {}
		...
		dialogRef.afterClosed().subscribe((result) => {
			if (Boolean(result)) {
				...
				result.product_type_id
					? (this.productobj["product_type_id"] = result.product_type_id ? JSON.stringify(result.product_type_id) : "")
					: delete this.productobj["product_type_id"];
				result.product_category_id
					? (this.productobj["product_category_id"] = result.product_category_id ? JSON.stringify(result.product_category_id) : "")
					: delete this.productobj["product_category_id"];
				...
				result.out_of_stock ? (this.productobj["out_of_stock"] = result.out_of_stock ? 1 : "") : 0;
				result.without_price ? (this.productobj["without_price"] = result.without_price ? 1 : "") : 0;
				result.without_barcode ? (this.productobj["without_barcode"] = result.without_barcode ? 1 : "") : 0;
	...
	getVariantData() {
		this.productService.GetProductList(this.productobj).subscribe((response: any) => {
	...
	getRowHeight({ product_name, product_type_name, product_category_name }) {
		if (
			(Boolean(product_name) && product_name.length > 55) ||
	...
	delete_product(): void {
		if (this.selected.length > 0) {
			let ids = this.selected.map(({ user_id: id, user_id, ...rest }) => ({
				id,
				user_id,
				...rest,
			}));
			const dialogRef = this.dialog.open(DeleteConfirmComponent, {
				data: {
					...
					message: "Are you sure want to delete following product?",
				},
			});
			//Call after delete confirm model close
			dialogRef.afterClosed().subscribe((result) => {
					...
					this.productService.deleteProductVariant(formdata).subscribe((response: any) => {
						if (response.success) {
							...
							_.remove(this.rows, function (o: any) {
								return Boolean(
									_.find(result, function (array) {
										return array.id == o.variant_id;
									})
								);
							});
	...
	AddNewTag(productdetail): void {
		if ((productdetail == "ALL" && this.selected.length > 0) || Boolean(productdetail.variant_id)) {
			let ids = this.selected.map(({ variant_id: id, variant_id, ...rest }) => ({
				id,
				variant_id,
				...rest,
			}));
			...
			dialogRef.afterClosed().subscribe((result) => {
				if (Boolean(result) && result.length > 0) {
					...
					this.productService.addTages(tagData).subscribe((response: any) => {
						this.utils.showSnackBar(response.message);
						result.forEach((tag_name) => {
							reference_ids.forEach((id) => {
								if (
									!Boolean(
										_.find(
											_.find(this.newrows, function (o) {
												return o.variant_id == id;
											}).tags,
											function (o) {
	...
	productStatusChange(product_id, evt) {
		...
		this.productService.productStatusChange(formData).subscribe((response: any) => {
			this.utils.showSnackBar(response.message);
		});
	}
	...
	GetProductList(pageSize, pageIndex) {
		...
		this.productService.GetProductList(this.productobj).subscribe((response: any) => {
			...
			if (response.success) {
				...
				this.adavance_filter = response.data.filters;
				this.rows = this.productsList;
				if (this.rows.length == 0 && this.productobj.pageIndex == 0) this.newrows = this.rows;
