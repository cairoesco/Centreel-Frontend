...
export class DiscountComponent implements OnInit {
  ...
  constructor(public dialog: MatDialog, private router: Router, private api: DiscountService, public utils: UtilsServiceService,private el: ElementRef,public fb: FormBuilder) { }
  ...
  discountStatusChange(po_id) {
    let formData = new FormData();
    formData.append('status',"completed");
    this.api.discountStatus(po_id,formData)
      .subscribe((response: any) => {
        if (response.success) {
          this.utils.showSnackBar(response.message);
          const targetIdx = this.discountData.map(item => item.id).indexOf(po_id);
          this.discountData[targetIdx].status ="completed";
        }
      });
  }
  ...
  uploadFile(event){
    this.router.navigate(['discount/po-list/create'], event);
  }
}
