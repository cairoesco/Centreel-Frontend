...
export class DiscountComponent {
	...
	constructor(public dialog: MatDialog, private discountService: DiscountService, private utils: UtilsServiceService, public refVar: ChangeDetectorRef) {}
	ngOnInit() {
		this.GetDiscounts();
	}
	...
	applyFilter(filterValue: string) {
		...
		if (this.rows.length > 0 && this.table) this.table.offset = 0;
	}
	...
	public deleteDiscount(discountId) {
		this.discountService.DeleteDiscount(discountId).subscribe((response: any) => {
			this.utils.showSnackBar(response.message);
		});
		this.GetDiscounts();
	}
	...
	ApplyMultipleFilter(fdata): void {
		...
		dialogRef.afterClosed().subscribe((result: any) => {
			if (Boolean(result)) {
				...
				this.discountService.GetDiscountFilterList(filterObject).subscribe((response: any) => {
					this.inProgress = false;
					if (response.success) {
