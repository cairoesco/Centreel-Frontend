...
@Component({
  selector: 'app-dailyinterim',
  templateUrl: './dailyinterim.component.html',
  styleUrls: ['./dailyinterim.component.scss']
})
export class DailyInterimComponent implements OnInit {
  console = console;
  inProgress: boolean = false;
  dailyInterimForm: FormGroup;
  storeList = [];
  resultdata = [];
  ...
  ngOnInit() {
    this.getStores();
    this.dailyInterimForm = this.formBuilder.group({
  ...
  onChanges(): void {
    ...
    this.dailyInterimForm.valueChanges.subscribe(val => {
      ...
      this.reportService.getDailyInterimReport(this.formobj)
  ...
  getExportPDF() {
    ...
    this.reportService.exportReport("daily", this.formobj).then(
      (res: HttpResponse<any>) => {
        this.reportService.downloadFile(res.body, 'application/pdf', 'Daily Interim ' + this.export_date);
      });
  }
  /* download pdf */
  ...
  getExportCSV(ext) {
    ...
    this.reportService.exportReport("daily", this.formobj).then(
      (res: HttpResponse<any>) => {
        if (ext == 'csv') {
          this.reportService.downloadFile(res.body, 'text/csv', 'Daily Interim ' + this.export_date);
        } else if (ext == 'xls') {
          this.reportService.downloadFile(res.body, 'application/vnd.ms-excel', 'Daily Interim ' + this.export_date);
        }
      });
  }
  /* download CSV */
    ...
    this.reportService.getStores()
      .subscribe((response: any) => {
        ...
        if (response.data.stores.length > 0) {
          this.dailyInterimForm.patchValue({ store_id: this.storeList[0].store_id });
        }
      });
  }

  reset_form() {
    this.dailyInterimForm.patchValue({ selected: { start: moment().format('DD/MM/YYYY'), end: moment().format('DD/MM/YYYY') } });
  }
}