...
@Component({
  ...
  styleUrls: ['./printable-menu.component.scss']
})
export class PrintableMenuComponent implements OnInit {
  ...
  ngOnInit() {
    this.getFilterData();
    this.printablemenu = this.formBuilder.group({
      store_id: [''], 
  ...
  onChanges() {
    ...
    this.printablemenu.valueChanges.subscribe(val => {
      ...
      if ((val.sort)) {
        ...
        this.param_dt =  '&sort=' + this.formobj.sort;
      } else {
        delete this.formobj.sort;
      }
      
      this.date_time = moment(val.selected.start, 'DD/MM/YYYY HH:mm:ss').format('YYYY-MM-DD HH:mm:ss');
      let params = 'store_id=' + this.formobj.store_id + '&type=' + this.formobj.type + '&date_time=' + this.date_time + this.param_dt; 
      ...
      this.reportService.getPrintableMenuReport(params)
  ...
  getExportPDF() {
    ...
    let params = 'store_id=' + this.formobj.store_id + '&type=' + this.formobj.type + '&ext=' + this.formobj.ext + '&date_time=' + this.date_time + this.param_dt; 
    
    this.reportService.printableMenu_exportReport(params).then(
      (res: HttpResponse<any>) => {
        this.reportService.downloadFile(res.body, 'application/pdf', 'Printable Menu '+this.export_date);
      });
  }
  /* download pdf */
  ...
  getExportCSV(ext) {
    ...
    (res: HttpResponse<any>) => {
      if (ext == 'csv') {
        this.reportService.downloadFile(res.body, 'text/csv', 'Printable Menu '+this.export_date);
      } else if (ext == 'xls') {
        this.reportService.downloadFile(res.body, 'application/vnd.ms-excel', 'Printable Menu '+this.export_date);
      }
    });
  }
  /* download CSV */
    ...
    .subscribe((response: any) => {
      if (response.status) {
        ...
        if (this.stores.length > 0) {
          this.printablemenu.patchValue({ store_id: this.stores[0].store_id });
        }
      }
      else {
  ...
  public sort = [{type_id:1, type_name:"Product Name"},{type_id:2, type_name:"Product Name"},
  {type_id:3, type_name:"THC CBD"},{type_id:4, type_name:"THC CBD"},
  {type_id:5, type_name:"Selling Price"},{type_id:6, type_name:"Selling Price"}];
