...
export class InventoryAuditComponent implements OnInit {
	...
	public product_type = [
		{ product_type_slug: "all", product_name: "All" },
		{ product_type_slug: "cannabis", product_name: "Cannabis" },
		{ product_type_slug: "non cannabis", product_name: "Non-Cannabis" },
	];
	public out_of_stock_array = [
		{ value: "all", item: "All" },
	...
	onChanges() {
		...
		this.reconcile.valueChanges.subscribe((val) => {
			...
			this.reportService.getInventoryAuditReport(params).subscribe(
				(response: any) => {
	...
	getFilterData() {
		this.reportService.getFilterList().subscribe((response: any) => {
			if (response.status) {
